...
public class open extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            request.getSession().setAttribute("year", c_year);
            request.getSession().setAttribute("semester", c_semester);
            //query
            String query = "SELECT * FROM `schedules`.`opencourse` WHERE `courseCode` = ? and `groupNo` = ? and `year` = ? and `semester` = ? and `department` = ?";
